      ...
      <div class="mainCon">
        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="sale in spuSaleAttrList" :key="sale.id">
                ...
                <dd
                  ...
                  :key="item.id"
                  @click="changeCheck(item, sale.spuSaleAttrValueList)"
                >
            ...
            <div class="cartWrap">
              <div class="controls">
                <!--                                              blur事件就是当 input 输入框失去焦点触发的事件回调                         -->
                <input
                  autocomplete="off"
                  class="itxt"
                  v-model="current"
                  @blur="changeCurrent"
                />
                <a @click="current++" class="plus">+</a>
                <!-- 如果用户点击 减少的话,进行判断,如果 当前数字小于等于1的话,就让他恒等一,其他情况可以继续减减 -->
                <a @click="current <= 1 ? current : current--" class="mins"
                  >-</a
                >
              </div>
              <div class="add">
...
export default {
  ...
  computed: {
    ...
    ...mapState({
      price: (state) => state.detail.detailList.price,
    }),
    imageList() {
  ...
  methods: {
    // 排他的回调函数,需要传入两个参数,第一个参数为 当前点击的那个对象,第二个参数为 整个数组
    changeCheck(saleItem, list) {
      // 我们先将整个数组的所有元素的 ischecked 属性设为 0
      list.map((item) => {
        item.isChecked = 0;
      });
      // 然后单独为 当前点击的这个元素的 ischecked 属性变为 1
      saleItem.isChecked = 1;
    },
    // 当失去焦点的时候执行的回调函数
    changeCurrent(event) {
      // 将用户输入的 内容 * 1  如果是正常的数字的话,就会得出结果,否则就是 NaN
      let result = event.target.value * 1;
      // 然后判断,当前那个数字是不是 NaN 或者用户输入的那个数字是不是小于 1(负数)
      if (isNaN(result) || result <= 1) {
        // 如果是的话,就将他重置为 1 ,不是的话就保持正常即可
        this.current = 1;
      } else {
        // 这里是为了以防万一,用户输入了小数,所以我们将其转换为整数后再返回
        this.current = parseInt(result);
      }
    },
    // 加入添加到购物车的回调函数
    async addShopCar() {
